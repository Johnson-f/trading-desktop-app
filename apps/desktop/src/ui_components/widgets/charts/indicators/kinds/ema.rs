...
use zaned_chart_core::{
    compute_ema, CandleData, ComputedSeries, InputSpec, LegendEntry, ParamField, ParamKind,
...
pub static SCHEMA: ParamSchema = ParamSchema {
    fields: &[
        ParamField {
            key: "period",
            label: "Period",
            kind: ParamKind::Int { default: 20, min: 1, max: 500 },
        },
        ParamField {
            key: "color",
            label: "Color",
            kind: ParamKind::Color { default: Rgba::from_rgb(78, 205, 196) },
        },
    ],
};
...
impl Indicator for Ema {
    ...
    fn target(&self) -> RenderTarget { RenderTarget::MainOverlay }
    ...
    fn draw_main(
        ...
        let Some(series) = computed.series.get("ema") else { return };
    ...
    fn legend(
        ...
        let Some(series) = computed.series.get("ema") else { return Vec::new() };
