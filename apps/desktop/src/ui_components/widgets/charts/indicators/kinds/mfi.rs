    ...
    fields: &[
        ...
        ParamField {
            ...
            kind: ParamKind::Color {
                default: Rgba::from_rgb(255, 87, 34),
            },
        },
    ],
};
...
impl Indicator for Mfi {
    ...
    fn compute(&self, inputs: &[&[f32]], params: &ParamValues) -> ComputedSeries {
        ...
        let volumes = inputs.get(3).copied().unwrap_or(&[]);
        let period = params.int("period").max(2) as usize;
        let series = compute_mfi(highs, lows, closes, volumes, period);
        let mut out = ComputedSeries::default();
        out.series.insert("mfi", series);
        out
    }
    ...
    fn draw_pane(
        ...
        let Some(series) = computed.series.get("mfi") else {
            return;
        };
