...
pub fn paint_crosshair(
    ...
    // Accept cursor in either the chart or volume pane
    let full_rect = Rect::from_min_max(chart_rect.min, volume_rect.max);
    ...
    // --- Horizontal line (only in the pane the cursor is in) ---
    let in_chart = chart_rect.contains(cursor_pos);
    let in_volume = volume_rect.contains(cursor_pos);
    ...
    if in_volume {
        let vol_painter = ui.painter_at(volume_rect);
        vol_painter.line_segment(
            [
                Pos2::new(volume_rect.left(), cursor_pos.y),
                Pos2::new(volume_rect.right(), cursor_pos.y),
            ],
            Stroke::new(0.5, CROSSHAIR_COLOR),
        );

        // Volume label on right edge
        let font = FontId::monospace(11.0);
        let vol_text = format_volume(candle.volume);
        let label_size = Vec2::new(60.0, 18.0);
        let vol_label_rect = Rect::from_min_size(
            Pos2::new(volume_rect.right() - label_size.x - 4.0, cursor_pos.y - label_size.y / 2.0),
            label_size,
        );
        vol_painter.rect_filled(vol_label_rect, 3.0, LABEL_BG);
        vol_painter.text(
            vol_label_rect.center(),
            egui::Align2::CENTER_CENTER,
            &vol_text,
            font,
            LABEL_TEXT,
    ...
    // --- Date label at bottom of volume pane ---
    ...
    if !date_text.is_empty() {
        ...
        let date_label_rect = Rect::from_min_size(
            Pos2::new(snapped_x - date_label_size.x / 2.0, volume_rect.bottom() - date_label_size.y - 4.0),
            date_label_size,
        );
    ...
    // --- OHLCV tooltip at top-left of chart ---
    let chart_painter = ui.painter_at(chart_rect);
