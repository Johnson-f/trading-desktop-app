...
pub fn compute_pivot_points(
    ...
    for sess_idx in 1..boundaries.len() {
        ...
        let pc = if prev_end > 0 && prev_end - 1 < n {
            closes[prev_end - 1]
        } else {
            continue;
        };
...
mod tests {
    ...
    fn standard_pivot_basic() {
        ...
        let (pivot, r1, _, _, s1, _, _) = compute_pivot_points(&highs, &lows, &closes, &dates, 0);
    ...
    fn no_levels_for_first_session() {
        ...
        let closes = vec![7.0f32; 8];
        let dates = make_dates(8, 2);
        let (pivot, _, _, _, _, _, _) = compute_pivot_points(&highs, &lows, &closes, &dates, 0);
    ...
    #[test]
    fn empty() {
        let (pivot, r1, r2, r3, s1, s2, s3) = compute_pivot_points(&[], &[], &[], &[], 0);
