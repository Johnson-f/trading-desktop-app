...
pub const ID: &str = "hma";
pub const NAME: &str = "Hull MA";
pub const LIKES: u32 = 12876;
    ...
    fields: &[
        ...
        ParamField {
            ...
            kind: ParamKind::Color {
                default: Rgba::from_rgb(0, 188, 212),
            },
        },
    ],
};
...
impl Indicator for Hma {
    ...
    fn display_name(&self, params: &ParamValues) -> String {
        format!("Hull MA({})", params.int("period"))
    }
    ...
    fn compute(&self, inputs: &[&[f32]], params: &ParamValues) -> ComputedSeries {
        ...
        let series = compute_hma(closes, period);
        let mut out = ComputedSeries::default();
        out.series.insert("hma", series);
        out
    }
    ...
    fn draw_main(
        ...
        let Some(series) = computed.series.get("hma") else {
            return;
        };
    ...
    fn legend(
        ...
        match series.get(idx).and_then(|v| *v) {
            Some(v) => vec![LegendEntry {
                label: format!("HMA{} {:.2}", params.int("period"), v),
